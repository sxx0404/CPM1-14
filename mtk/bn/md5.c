...
void MD5Update (MD5_CTX *mdContext, const void *inData, unsigned int inLen)
{
	UINT4 in[16];
	int mdi = 0;
	unsigned int i = 0, ii = 0;
	const unsigned char *inBuf = inData;
...
void Md5Data(const void *inData, unsigned int iLength, uint8_t *Md5Buf)
{
	MD5_CTX m_md5;
	MD5Init(&m_md5, 0);
	MD5Update(&m_md5, inData, iLength);
	MD5Final(&m_md5);
	memcpy(Md5Buf, m_md5.digest, 16);
}
